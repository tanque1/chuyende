...
export default function ModalAddRoom({
  ...
  const handleSubmit = () => {
    const usersChecked = data.filter((d) => d.checked);
    onSubmit && onSubmit(usersChecked);
    onClose && onClose();
  };
  useEffect(() => {
    setData(users);
  ...
  return (
    <ModalContainer onClose={onClose} ignoreContainer={true} visible={visible}>
                  ...
                  <div key={u.sub} className="flex justify-between items-center mb-3">
                    <div className="participant-img-name-wrap display-center cursor-pointer">
                      <div className="participant-img flex justify-center items-center">
                        ...
                        />
                        <div className="participant-name ml-2">{u.name}</div>
                      </div>
                    </div>
                    <div className="flex items-center">
                      <input
                        onChange={() => handleOnChange(u)}
                        id="checked-checkbox"
                        type="checkbox"
                        name={u.name}
                        value={u.checked}
